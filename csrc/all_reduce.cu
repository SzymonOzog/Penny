...
__global__ void all_reduce(scalar_t *destination, scalar_t* buffer, int peer) 
{
...
void all_reduce(torch::Tensor& buffer, int world_size, int local_size) 
{
    ...
    all_reduce<<<grid_size, BLOCK_SIZE, 0, stream>>>(destination, static_cast<half*>(buffer.data_ptr()),
